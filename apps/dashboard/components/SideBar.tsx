...
const SideBar = ({ role, userId, token }: SideBarProps) => {
  ...
  const { data, isLoading } = useGetUserById({ userId: Number(userId) });
          ...
          return (
            hasPermission(route.permission, role) && (
              <div
                key={route.href}
                className={cn(
                  "flex items-center gap-2.5 p-2.5 rounded-md font-medium hover: text-primary transition text-slate-700 dark:text-slate-300 cursor-pointer",
                  isActive &&
